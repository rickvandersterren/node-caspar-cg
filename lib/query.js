	...
	ccg.prototype.getMediaFiles = function (cb) {
		...
		self.sendCommand("CLS", function (err, data) {
			...
			for (var index in files) {
				...
				file = /"([^"]*)"[\s]+([^\s]*)[\s]+([^\s]*)[\s]+([^\s]*)\s([\d]+)\s([\d\/]+)?/.exec(file);

				out.push({
					file: file[1],
					type: file[2],
					length: file[3],
					lengthInFrames: parseInt(file[5]),
					lengthInMS: parseInt((file[5]/25)*1000),
					date: new Date(
		...
		self.sendCommand(command, function (err, data) {
			...
			if (data.substr(0, 1) == "<") {
				xml(data, function (err, data) {
					...
					data.layers = (Array.isArray(data.layers)) ? data.layers : [data.layers];
					delete data.stage;
